...
页面按用途分为四个分页,每屏只显示相关内容,不用在超长页面里来回滚动:

| 分页 | 内容 |
|---|---|
| **记录** | 今日骑行打卡 + 骑行数据录入表单 + 本次评分(评分卡与雷达图) |
| **看板** | 评分趋势 + 速度曲线 + 海拔曲线 + 骑行轨迹地图 |
| **历史** | 历史记录表格(筛选、编辑、重命名、导出 JSON / CSV) |
| **单车与轮胎** | 单车与外胎寿命管理(该分页标签上会显示外胎异常提醒的红/黄点) |
...
- **今日骑行打卡**:记录「今天是否骑行」,这是日常使用的主要入口
  - 选「骑了」→ **选择单车**(必填)+ 距离(可留空)→ 打卡:自动生成一条当天的骑行记录并关联该车,
