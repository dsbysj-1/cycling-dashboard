...
顶部是「今天骑了吗」快速打卡(选择单车、可选距离,打卡后自动同步到单车管理),下方是详细录入表单;
右列为本次评分、路线评价(含他人评价预留区)、近 30 天概览与保养提醒。

![记录页](docs/screenshots/01-record.png)

### 看板页:图表、轨迹与历史

评分趋势(含评分档位分区)、速度曲线与海拔曲线、骑行轨迹地图与评分雷达,底部是历史记录表格(支持筛选、编辑、路线编号重命名与 JSON / CSV 导出)。

![看板页](docs/screenshots/02-dashboard.png)

### 单车与轮胎管理

单车支持自定义名称与类别,每辆车可选择外胎类型(各有固定建议寿命),按该车累计里程跟踪磨损,
达到建议寿命会提示检查外胎状态;列表显示累计里程、骑行次数与最近骑行日期。

![单车与轮胎](docs/screenshots/03-bikes.png)

### 明暗(昼夜)主题

顶栏可一键切换日间 / 夜间主题,界面配色、图表网格线与高德地图底图都会跟随,选择会记住在浏览器里。

![浅色主题](docs/screenshots/04-light-mode.png)
